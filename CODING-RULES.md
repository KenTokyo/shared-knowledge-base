...
- **Unsichtbare Partikel zuerst am Renderpfad prüfen (2026-06-07, PFLICHT):** Bei Instanced-Partikeln nicht zuerst Anzahl, Größe oder Alpha aufblasen. Prüfe in dieser Reihenfolge: `instanceColor` wird vor dem ersten Render angelegt und `needsUpdate` gesetzt, Shader liest `instanceColor` explizit, `drawRange`/Geometry ist gültig, `depthWrite=false`, Fog/ToneMapping frisst die Farbe nicht weg, flache Sprites sind zur Kamera sichtbar ausgerichtet und Glow ist ein Akzent statt einziger Sichtbarkeit.
