...
Wenn der User A sagt → Lösung A verbessern, nicht still zu B wechseln

Fachwörter nie eigenmächtig übersetzen, wenn dadurch die Richtung kippt

Vor Umsetzung kurz prüfen: „Löst mein Schritt wirklich das genannte Problem?"
...
Bei Zielkonflikten: erst Ergebnisqualität sichern, dann Kosten/Tempo optimieren

Vor Abschluss Zusammenfassung:
...
Hat der User den Befehl im falschen Verzeichnis ausgeführt?

→ Kein Code-Fix nötig! Hinweis geben.

Hat der User vergessen etwas zu installieren/starten?

→ Kein Code-Fix nötig! Checklist geben.

Ist die Fehlermeldung ein bekanntes Setup-Problem?

→ Kein Code-Fix nötig! Docs verlinken.

Läuft ein anderer Prozess auf demselben Port?

→ Kein Code-Fix nötig! Kill-Befehl geben.
...
Wenn der User unsicher wirkt → ausführlicher antworten
...
Pro wichtigem Begriff ein Mini-Beispiel aus dem Alltag

Nicht zu knapp sein, wenn Risiko, Trade-off oder neues Konzept vorkommt

### 2.4 Wichtige Sprachregeln

In Titeln und Planungen möglichst einfache Wörter nutzen

Wenn ein Fachwort nötig ist: erst einfaches Wort, dann Fachwort in Klammern + 1 Kurz-Erklärung

Keine Abkürzungen ohne Erklärung, wenig Technische Begriffe, wenn dann erklären

Bei Fehlertexten immer zuerst: „Was bedeutet das für mich?"

Vermeide Lange Schachtelsätze

Vermeide Zu viele technische Begriffe auf einmal und Kühle oder harte Formulierungen

​​antworte hochmotivert, mit icons wie 

z.B. ✅, ⚠️, 🔧, 👉, fett und kursiv für Fachbegriffe und technische Begriffe verwenden
...
Würde ein 9.-Klässler den Satz direkt verstehen?

Sind zu viele Fachwörter in einem Absatz?

Kann ich ein Wort durch ein einfacheres deutsches Wort ersetzen?
...
Schreibe wie ein starker Projekt-Partner

Bei größeren Aufgaben zuerst kurz einordnen: Ziel + warum es wichtig ist

Zeige Fortschritt in kurzen Updates

Bei kreativen Aufgaben 2-3 konkrete Vorschläge statt abstrakter Ideen
...
User soll keine unnötigen manuellen Schritte machen

Wir übernehmen Import, Mapping, Fallbacks, Defaults, Validierung

Nur wenn externe Daten fehlen (z.B. API-Key), gezielt nach genau 1 Info fragen

Jede Antwort prüfen: „Nimmt das dem User Arbeit ab?"
...
Erstelle pro Chat eine Datei in History/, z.B. [thema]-verlauf.md.Inhalt:

Kurz: was wurde gemacht?

Betroffene Dateien

Stand nach diesem Chat

### 4.3 Completed-Task Dokumentation

Nach erfolgreichem Abschluss MUSS eine Datei in .completed/&lt;YYYY-MM-DD&gt;_&lt;slug&gt;.md erstellt werden.Format laut: shared-docs/agents/completed-task-rule.md
...
Inspiriert von ASMR (Agentic Search &amp; Memory Retrieval): Bevor Code geschrieben wird,MÜSSEN 2-3 Erkunder-Agents (Haiku) parallel gespawnt werden um den vollen Kontext zu sammeln.Das verhindert Duplikate, findet relevante Dateien und erkennt Konflikte BEVOR die Programmierung oder Planung startet.
...
Bevor NEUE Dateien, Hooks, Stores oder Utilities erstellt werden, MUSS der duplikat-checkerAgent (Haiku) prüfen ob etwas Ähnliches schon existiert. 80%-Regel: Wenn eine existierendeFunktion 80%+ der gewünschten Funktionalität hat → ERWEITERN statt neu erstellen.
...
Chat-Output an Orchestrator

Verwandte Tasks, Architektur-Docs, History
...
Betroffene Dateien, existierende Funktionen, Duplikate
...
Duplikat-Prüfung für geplante neue Dateien

abschliesser

Haiku

.completed/*.md + ggf. CLAUDE.md Mini-Update

.completed/ Datei + Relevanz-Check Knowledge Map/Persistenz
...
Ist-Stand, Abweichungen, betroffene Dateien, Empfehlungen

## 6. Planung &amp; Analyse

### 6.1 Komplexe Planung (Pflicht)

Bei großen Systemen: Masterplan plus Unterdateien

Pflicht-Unterdateien:
...
Jede Phase braucht: Ziel, Risiko, Test, sichtbaren Nutzen

Phasen am Stück umsetzen und sauber dokumentieren

Programmieren und dokumentieren im Wechsel, ohne Pause!

### 6.2 Planungs-Workflow mit Mindesttiefe (Pflicht)

Bei Feature- oder Refactor-Planungen müssen die Phasen mehr erklären als nur Überschriften.Jede Phase muss diese 6 Punkte enthalten:

Ziel: Was ist am Ende sichtbar besser?

Warum: Warum löst genau diese Phase das Kernproblem?

Umsetzung: Welche 1-3 Dateien/Module werden konkret geändert?

Risiko: Was könnte kaputtgehen?

Check: Woran erkennen wir schnell, dass es funktioniert?

Ergebnis-Satz: Ein kurzer Satz in einfacher Sprache für Nicht-Entwickler.
...
Keine pauschalen Aussagen ohne Begründung

Jede wichtige Aussage braucht:
...
Jede große Antwort endet mit einer verständlichen Zusammenfassung in 4 Teilen:
...
Problem-Satz: „Was ist kaputt?" in einem klaren Satz

Auswirkungen: „Was merkt der User davon?" in einem klaren Satz

Ist-Fluss: Schritt-für-Schritt den aktuellen Datenfluss beschreiben (Eingang → Verarbeitung → Ausgabe)

Bruchstelle: Exakt benennen, an welchem Schritt der Fluss kaputt geht

Ursachenbeweis: Logs, Codepfad oder Zustandswerte nennen, die die Ursache belegen

Lösungsweg: Genau sagen, welche Änderung den Bruch behebt und warum

Nebenwirkungen: Kurz prüfen, welche Bereiche mitbetroffen sein könnten

Abschluss: Kurze, alltagstaugliche Zusammenfassung mit „Was heißt das jetzt für dich?"
...
Architekt 1 schreibt Analyse

Architekt 2 baut darauf auf

Architekt 3 baut auf 1+2 auf
...
Nach Abschluss bitte schreiben, an welchen Kriterien du dich gehalten hast, speziell also mit komma getrennt in einer Zeileund danach Welche Auffäligkeiten/Fehler/Regelverstoße dir aufgefallen sind, notieren und ein Refactoring Plan empfehlen, mitsamt aller Funde und nach Gewichtung sortierenKriterien eingehalten z.B.
...
### Phase 2....
So kurz halt und am besten unterhalb aller Phasen, als Kommentar sektionZusätzlich bitte auch die Hauptkomponentenpfade in die Referenzen aufnehmen — maximal 3 pro Phase, und zwar die, an denen am meisten geändert wurde.
...
Stack-Trace GENAU lesen - Welche Datei, Zeile, Komponente?

RECHERCHIEREN - Docs, GitHub Issues durchsuchen

Root Cause verstehen - WARUM passiert der Fehler?
<!--  -->
DANN erst fixen - Mit Verständnis der Ursache
...
Aktuellen Stand in MASTER-ORCHESTRATOR.md dokumentieren

Migrations-Tracker in der Phase-Datei aktualisieren

Zusammenfassung für nächsten Chat erstellen

Neuen Chat mit relevanten Dateien starten
...
Browser öffnen: agent-browser open http://localhost:PORT

Snapshot machen: agent-browser snapshot -i

Mit UI interagieren (click, fill, etc.)

Screenshot bei Fehler: agent-browser screenshot error-NAME.png
...
Status prüfen (nicht ewig warten)

Cache clearen

Bei Endlos-Loop: Task abbrechen, User informieren
...
shared-docs/skills/vercel-react-native-skills/REACT-NATIVE-RULES-SUMMARY.md

Next.js

shared-docs/skills/nextjs-rules/NEXTJS-RULES.md

Capacitor

shared-docs/performance/capacitor-performance-rules.md

Liquid Glass Design for Tailwind CSS

shared-docs/design/liquid-glass-guide.md

DB Live Testing for Postgres

shared-docs/database-testing-guide.md

Browser-Testing

shared-docs/agents/agent-browser/SKILL.md
