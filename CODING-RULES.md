...
- For screenshots, reuse an existing or in-app browser tab/native capture first; otherwise use a visible local test browser. Before any necessary headless run, warn the user about CPU cost and state the reason, alternative and cleanup deadline. Forced software rendering for ordinary 3D captures, unannounced headless starts and test browsers left running after completion are rule violations. Stop only your own offending run, then switch to the suitable capture path. This covers Playwright, direct CDP, shell scripts and other browser launchers. Follow [SCREENSHOT-GUIDE.md](SCREENSHOT-GUIDE.md); no automatic capture-framework project. Typechecks and stub-DOM checks do not prove visual quality or real runtime behavior.
