...
- **MUSS: Normierte Achsenparameter klemmen, bevor sie potenziert werden.** Ein aus einer Vertexkoordinate gerechneter 0..1-Laufparameter (`const t = (y + half) / height`) ist **nie** exakt 0 oder 1: `y` kommt aus dem **Float32**-Puffer der Geometrie zurueck, `half` und `height` sind **Float64** — an den Kanten heben sie sich nicht auf, `t` landet bei rund `-1e-8`. Danach ist `Math.pow(t, 0.6)` **NaN**, und ebenso `Math.sqrt`, `Math.log`, `Math.asin`/`acos` ausserhalb ihres Definitionsbereichs. Solche `t` immer hart auf 0..1 klemmen (`src/components/3d/shared/normalizedAxisParam.ts` → `axisParam01(abstand, spanne)`), nicht nur die Stelle flicken, die heute knallt. **Fuenf Schreibweisen, dieselbe Falle** — die drei letzten laufen von der KANTE zur Mitte und sehen deshalb aus, als haetten sie schon eine Obergrenze: `(y + half) / height` · `y / height + 0.5` · `1 - Math.abs(x) / halfW` · `1 - Math.abs(x / halfW)` · `1 - x / halfW`. Bei den gespiegelten Formen liest `x` an der Aussenkante als Float32 minimal **groesser** als `halfW` zurueck, der Quotient wird `1 + 4e-8`, und `1 - …` kippt negativ — identischer Knall. Gemessen am 2026-07-24 in `makeHeatRidge` (`ashenJuggernaut`) an den ECHTEN Leistenbreiten: 2 von 3 Leisten je 24 NaN-Positionen. **Sechste, indirekte Form:** dieselbe Division steckt hinter einem lokalen Helfer (`const norm = (v, ext) => v / ext;` … `const t = norm(y, halfH);`) — in der Zeile matcht dann kein Muster. Ist der Helfer **ungeklemmt** und sein Ergebnis wird danach potenziert, meldet ihn die statische Regelpruefung (`--nur-regel`) trotzdem; ein **geklemmter** Helfer (`unitSpan` in `krustenGraeber`, `t < 0 ? 0 : t > 1 ? 1 : t`) bleibt korrekt stumm. **Der Fehler haengt nicht an der Groesse:** Float32 hat rund `6e-8` *relative* Genauigkeit, der Rest ist also bei einer Hoehe von `0.3` genauso gross wie bei `2.4` — entscheidend ist allein, ob `height/2` in Float32 exakt darstellbar ist (`0.5` ja, `0.92`/`0.72`/`0.3`/`0.66` nein). **Provably harmlos und deshalb ausgenommen:** `(y - minY) / span`, wo beide Seiten aus demselben Float32-Puffer stammen. **Folge sonst — zwei Symptome, eine Ursache:** Eine Bounding-Sphere mit Radius `NaN` besteht keinen Frustum-Test (`intersectsSphere` vergleicht gegen `NaN`, liefert immer `false`) ⇒ das Objekt wird **nie gezeichnet**, waehrend HUD, Lebensbalken und Trefferzonen normal weiterlaufen. Zieht der Guard die Werte auf `0`, rutschen einzelne Vertices auf den lokalen Ursprung, ihre Nachbarn bleiben stehen ⇒ ein langgezogenes **schwarzes Dreiecksband** quer durchs Bild. Gate dafuer: `pnpm geometry:nan-probe` (offline, ohne Browser), Frueherkennung eine Stufe davor mit `pnpm geometry:nan-probe -- --pow`.
...
- **MUSS: Stein-/Material-Look — nie untexturierte Flat-Color auf Hero-Flächen (User-Order 2026-07-14, L13-Learning).** Große sichtbare Umgebungsobjekte (Häuser, Tore, Statuen, Felsen, Terrassen, Dächer) dürfen NICHT als reines `meshStandardMaterial` mit nur einer Farbe stehen bleiben: nachts/lichtabgewandt liest jede solche Fläche als tote schwarze Silhouette „ohne Körnung". Pflicht ist Detail-Grain (multiplikative helle Diffuse-Map) + Normal-Map + Ton-Varianz — als EIN geteiltes, einmal gebacktes Textur-Set pro Look statt Per-Batch-Bakes (Muster: `voxel-samurai-quiz/src/components/3d/environment/stoneDetailTexture.ts`, Repeat-Stufen aus dem Cache, Draw-Call-Zahl unverändert). Merksatz: Die Diffuse multipliziert mit der Materialfarbe → Basistöne beim Umstellen ~15 % anheben, sonst dunkelt alles ab.
...
- **MUSS: Nie ein Licht in eine Gruppe hängen, deren `visible` zur Laufzeit umschaltet.** three.js sammelt die sichtbaren Lichter pro Frame; ändert sich ihre **Anzahl**, kippt der Lights-State-Hash und **jedes Material der gesamten Szene** bekommt ein neu kompiliertes Shader-Programm. Ein Licht in einer phasenweise ein-/ausgeblendeten Gruppe löst das also mehrmals pro Kampfzyklus aus — Symptome sind harte Ruckler, eine Flut `GL_INVALID_VALUE: glGetProgramiv: Program object expected` (der Treiber gibt irgendwann keine Programm-Slots mehr aus) und am Ende ein schwarzes Bild. Richtig: Licht dauerhaft montiert lassen und nur `intensity` auf 0 fahren; die **Meshes** dürfen weiter umschalten. Gleiches gilt für Lichter unter einem Root, dessen `visible` pro Frame gesetzt wird — dort mindestens sicherstellen, dass es nur bei Spawn/Tod passiert, nicht zyklisch.
- **MUSS: Bei geteiltem `onBeforeCompile` immer `customProgramCacheKey` setzen.** `Material.customProgramCacheKey()` gibt per Default `this.onBeforeCompile.toString()` zurück (three r184: `three.core.js:20936`) und geht in den Programm-Schlüssel ein (`three.module.js:7802`). Bauen mehrere Materialien ihren Shader über **denselben** Helfer, ist ihr Funktionstext identisch — der unterscheidende Chunk steckt unsichtbar in der Closure. Materialien mit gleichen Grundparametern (gleiches `flatShading`, gleiche Maps …) teilen sich dann still **ein** Programm, und alle außer dem zuerst kompilierten tragen den **falschen** Shader. Kein Crash, kein Fehler im Log — nur stumm falsche Optik. Fix: `mat.customProgramCacheKey = () => "<owner>:" + bodyChunk`. Den unterscheidenden Chunk selbst als Schlüssel nehmen, dann kann der Wert nie auseinanderlaufen.
- **CHECK: Die Abdeckung mit einem AST-Scan beweisen, nicht mit `grep` zählen (Learning 2026-07-22).** „N Dateien mit `onBeforeCompile`, N mit `customProgramCacheKey`" beweist **nichts** — der Key gilt je **Material**, und ein Helfer bedient viele. Ein Regex-Scan versagt zusätzlich, weil der Key meist **als Parameter** hereinkommt (`mat.customProgramCacheKey = () => cacheKey`) und nicht als Literal an der Zuweisung steht; er meldet dann falsch „0 Duplikate". Belastbar sind nur zwei AST-Prüfungen: **(1) Pairing** — bekommt jeder Empfänger im selben Scope beides? **(2) Eindeutigkeit** — jeden Key über die Literale der Aufrufstellen auflösen und repo-weit auf Duplikate prüfen. Der teuerste Fall ist dabei unsichtbar für beide naiven Zählungen: **eine** schlüssellose Helfer-Zuweisung, die N-mal aufgerufen wird, legt N Materialien auf ein Programm. Gegenprobe: Aufrufe des Helfers zählen. **Kein Key nötig**, wenn der Injector seine variablen Werte als `shader.uniforms.*` durchreicht — dann ist der GLSL-Text konstant und geteiltes Programm korrekt.
- **MUSS: Normalisierte Laufvariablen aus Vertex-Koordinaten klemmen (`0..1`), bevor `Math.pow` mit gebrochenem Exponenten darauf läuft.** Vertex-Koordinaten liegen als **float32** im Attribut, die Rechnung läuft in float64: `float32(-0.37) / 0.37` ergibt `-1.0000000128`, nicht `-1`. Ein daraus gebildetes `t` kippt minimal unter null, und `Math.pow(negativ, gebrochen)` ist **NaN**. Ein einziger NaN-Vertex vergiftet die ganze BoundingSphere (`computeBoundingSphere(): Computed radius is NaN`) und nimmt dem Mesh das Frustum-Culling. Immer über einen Helfer wie `unitSpan(value, halfExtent)` gehen, der auf 0..1 klemmt — kostet nichts, macht die Formeln hart.
- **CHECK: Prozedurale Geometrie mit einem Skript gegen NaN prüfen, statt den Code zu überfliegen.** Ein kleines Node-Skript, das jede Builder-Funktion mit den **echten** Argumenten des Modells aufruft und das `position`-Attribut auf `Number.isFinite` prüft, findet die kaputte Funktion in Sekunden und beweist danach die Reparatur. Muster: `voxel-samurai-quiz/scripts/qa/check-apex-geometry-nan.mts`.

**Vorfall-Merkhilfe (2026-07-21, Apex-Jäger L12 „Glas-Wüste" — schwarzes Bild + Dauer-Lag):**
- Sichtbares Problem: Spieler meldet „laggt unglaublich", „ich sehe nur noch schwarz", benannt an **einer** von drei neuen Kreaturen. Log: 2× `computeBoundingSphere(): Computed radius is NaN … BoxGeometry`, 256× `GL_INVALID_VALUE: glGetProgramiv: Program object expected`.
- Falsche Fährten: Partikel-Budget, Mesh-Anzahl, `dispose()` im `useEffect`-Cleanup unter StrictMode (das Muster nutzen alle Bossmodelle des Projekts unauffällig — es heilt sich beim nächsten Frame selbst).
- Ursachen (drei, alle in derselben Kreatur): (1) `<pointLight>` **innerhalb** einer Gruppe, deren `visible` pro Kampfphase umschaltete → Lights-Hash kippt → Vollszenen-Shader-Rekompilierung → Programm-Slots erschöpft → schwarz. (2) `Math.pow(t, 1.4/1.6)` auf ungeklemmtem `t` aus float32-Koordinaten → NaN-Geometrie. (3) Geteilter `onBeforeCompile` ohne `customProgramCacheKey` → Materialien trugen fremde Shader-Chunks.
- Merksatz: Bei „schwarz + Lag + Programm-Fehlerflut" **zuerst** zählen, ob sich die Anzahl sichtbarer Lichter pro Frame ändert. Das ist die teuerste stille Katastrophe in three.js — ein einziges falsch platziertes Licht kostet die ganze Szene.
...
- `pnpm geometry:nan-probe` - offline NaN-Check aller statisch gebauten Geometrien (Bosse, Gegner, Map-Bausaetze). Nennt Datei und Export statt nur `BoxGeometry`. Der Lauf hat **zwei Pruefwege mit unterschiedlicher Reichweite**:
  1. **Ausfuehrung** — baut Geometrie und misst echte Werte, erreicht aber nur, was sich aus einem Modul heraus aufrufen laesst. Schalter: `-- --pow` (Frueherkennung: findet `Math.pow/sqrt/log/asin/acos` mit nicht-endlichem Ergebnis, bevor ein NaN im Positionsattribut landet), `-- --wide` (alle `.ts` statt nur `*Geometry.ts`; reproduziert nebenbei Three-Materialwarnungen offline **mit Dateinamen**), `-- --synth` (ruft auch Fabriken mit Parametern, mit erfundenen Kennwerten `0.35/1/2.4`).
  2. **Statische Regelpruefung** — liest statt auszufuehren, laeuft immer mit und erreicht damit auch `.tsx`. Das ist die einzige Abdeckung fuer die **21 Boss-Modelle, die ihren Bausatz in einem `useMemo` in `*Model.tsx` bauen**: dieser Rumpf laeuft beim Import nicht, kein Ausfuehrungslauf betritt ihn je. Schalter: `-- --nur-regel` ueberspringt den Ausfuehrungslauf und prueft **nur** statisch — Sekunden statt Minuten, deshalb der richtige Schnellcheck nach einer Geometrieaenderung.
  - ⚠️ **`--synth`-Funde immer gegen die echten Aufrufwerte gegenrechnen**, bevor man sie als Produktionsfehler behandelt — die Kennwerte sind erfunden. Bei der Normierungsfalle war der Fund echt (der Fehler ist skalenunabhaengig), bei einer Fabrik mit fachlicher Bedeutung (z. B. `teeth = 0.35` als Zahnzahl) war er ein Artefakt. Meldet die Sonde `ANZAHL-VERDACHT: nur bei Satz 0.35 kaputt, bei 1/2.4 sauber`, hat sie diesen Artefakt selbst erkannt: ein Parameter ist eine Stueckzahl, ein Bruch < 1 floort auf 0 Segmente und teilt durch null.
  - ⚠️ **`--synth` ist ein Suchmodus, kein Gate.** Der Lauf endet wegen genau dieses bewiesenen Artefakts (`makeMawLip`, `eclipseOni`) dauerhaft mit Exit 1. Wer ihn als CI-Gate verdrahtet, bekommt ein permanent rotes Gate. Gate-Laeufe sind der Standardlauf und `--nur-regel`.
...
- **Objekt fehlt ganz, obwohl HUD/Lebensbalken/Trefferzonen laufen — oder ein schwarzes Band zieht quer durchs Bild:** Zuerst NaN in den Positionsdaten prüfen, nicht Material, Licht oder Culling-Werte drehen. Beides sind dieselbe Ursache (NaN-Bounding-Sphere bzw. auf `0` geheilte Einzelvertices). `pnpm geometry:nan-probe` läuft ohne Browser; der Laufzeit-Wächter `src/lib/three/nanGeometryGuard.ts` nennt im Strict-Modus den Bauort, auch wenn der Fehler erst im Renderer auffällt.
...
- **MUSS: Kamera-Shake ist ein IMPULS, kein Skalar — und wirkt NACH `lookAt` (Learning 2026-07-25, voxel-samurai-quiz Kamera-Wucht).** Ein einzelner State-Wert, der gleichzeitig Stärke und Restzeit ist und linear mit `dt` herunterläuft, kann keine Kurve tragen: `0.72` rüttelt 0.72 s fast konstant und bricht dann hart ab — das liest als Erdbeben, nicht als Schlag. Richtig ist ein **gepoolter Impuls** je Ereignis mit eigener Dauer, Frequenz, Hüllkurve (`pow(1-t, decayPower)`) und Richtung; die Summe wird gedeckelt (nicht `max()`, sonst wiegt ein Fünffachtreffer wie ein Einzeltreffer). Zwei Fehler, die den Effekt komplett entwerten: (1) **`Math.random()` pro Frame** — weißes Rauschen liest als Bildfehler; zwei Sinus mit irrationalem Frequenzverhältnis kosten dasselbe und lesen als Erschütterung. (2) **Positionsversatz VOR `camera.lookAt()`** — die Blickkorrektur rechnet ihn fast vollständig weg, sichtbar bleibt nur ein Zittern des Orbitwinkels. Der wirksame Kanal ist **Rotation** (`rotateX/Y/Z` nach `lookAt`, Basis aus `camera.quaternion`), Translation ist nur Beigabe. Referenz: `src/lib/juice/cameraShakeRuntime.ts`. Nutzerstärke ist Pflicht (Motion-Sensitivität): ein Regler `0…1.5` skaliert die Ausgabe, `0` = völlig ruhiges Bild.
- **MUSS: Wucht auch ohne Treffer.** Ein Shake, der nur bei `hitCount > 0` feuert, lässt jeden Leerschwung tot wirken — der Schwung selbst ist ein Ereignis. Cast/Release bekommt einen kurzen gerichteten `kick` (mit der Schwungrichtung), der Kontakt einen `punch` (Rückstoß vom Einschlag) obendrauf. Angehängt wird das an **einen** Funnel pro Klasse, nicht pro Skill.
...
- **MUSS: Schwerer Boden-Impact = geschichteter Druckstoss.** Flash + Debris + KREISRUNDER Schockring (Bandradien konstant — Kreis-Kontur nie per Winkel-Sinus verformen, wavy liest als Pfuetze) + Windstoss-Linien (duenne C-Bogen-Wisps, auswaerts berstend + tangentialer Umlauf) + Rauch + Boden-Scorch (CD-gegatet, FIFO-Cap) — alles aus bestehenden Pools. Ein schwerer Impact ohne Wind- und Scorch-Schicht liest als "nur Licht" (Kaskadenbrecher Runde 3, 2026-07-17).
...
## 21. FPS-/Game-Runtime-Muster (wiederverwendbar)

Praxis-Learnings aus dem First-Person-Quiz-Shooter (QBA-Feedback 11/12) — gelten für alle Spiel-Runtimes (HUD, Movement, Skills, Sound, Stationen). Kern-Prinzip: **bestehende Zustände/Flüsse wiederverwenden, deterministisch bleiben, kein neuer Loop/State ohne Not.** (Früher in `CODING-RULES.md`, hierher als Game-Learning ausgelagert.)
