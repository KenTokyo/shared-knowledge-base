...
- **MUSS: Rauch-Strömungs-Standard einhalten (projektübergreifend).** Volumetrischer/CPU-Rauch bewegt sich per **divergenzfreier Curl-Noise-Advektion** (Auftrieb + Fall + Curl-Turbulenz + abklingender Wind + exp-Drag, allok-frei in ein Temp), NIE per Sinus-Render-Wackler (der koppelt an den Pool-Slot und glitcht bei FIFO-Reuse). Danach **Boden-Clamp**: `if (pos.y < floorY) { pos.y = floorY; if (vel.y < 0) vel.y = 0; }` mit `floorY ≈ spawn.y − 0.4` → Rauch sackt nicht durchs Terrain. Farbe = **2-Stop-Rampe** `mix(deep, core)·lit` + optionaler Hot-Core (`temp`), NIE Grau-Lift `(base+x)·y` (entsättigt zu Grau); NormalBlending, kein `uGlow` (Rauch überstrahlt nie). Referenz-Impl + Klassen-Doktrin: `prompts/vfx-skills-prompt.md` (Doktrin „Rauch-Plume-System", Abschnitt A).
