...
- **MUSS: Per-Frame mutierte Transforms tragen kein konkurrierendes JSX-Transform-Prop.** Wenn eine Komponente re-rendert (Store-/State-/Prop-Change) und ein Mesh/Group seinen `position`/`rotation`/`scale` jeden Frame über `ref.current` setzt, darf dasselbe Transform NICHT zusätzlich als JSX-Prop (`<mesh rotation={...}>`) deklariert sein — sonst überschreibt React es beim Re-Render mit dem statischen Prop-Wert (sichtbares Reset-Flackern des animierten Teils). Statischen Anteil (z.B. Radialwinkel) auf einen äußeren Pivot legen, den animierten Anteil rein per Ref mutieren. Ausnahme: Props, die der `useFrame`-Pfad ohnehin jeden Frame frisch überschreibt (z.B. `group.position`). QBA-Boss-Learning: die pro Frame gekippten Kronen-/Klingen-Scharniere flackerten bei jedem HP-/State-Change zurück, bis das `rotation`-Prop entfernt wurde.
