...
- **Kein Standard-Bootstrap-Look:** Vermeide 08/15 Layouts. Nutze Asymmetrie, Overlays oder moderne Grid-Bento-Designs.

## 🎬 Animationen (Pflicht)

- Falls nicht vorhanden: baue `components/ScrollReveal.tsx` mit **React Intersection Observer**.
- Alle Sektionen ausser Hero mit ScrollReveal animieren (Fade + leichter Blur + kleine Y-Translation).
- Stagger/Waterfall je Sektion nutzen (verzoegertes Einblenden von Child-Elementen).
- Hero **nicht** mit Intersection Observer animieren:
  - On-Load Animation mit links/rechts einfahrenden Elementen
  - Waterfall-Delays fuer Headlines, Subline, CTA
- `prefers-reduced-motion` respektieren (Animationen reduzieren/deaktivieren).

## 🧬 Anti-Clone Check (Pflicht)

- Vor Abschluss aktiv pruefen: wirkt die Komponente wie ein bereits bekanntes Standard-Template?
- Wenn ja: sichtbar umbauen, bis ein klar eigener Look entsteht.
