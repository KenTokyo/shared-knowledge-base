...
Build an MCP server to use with ChatGPT Apps, deep research, or API integrations.
...
In this guide, we’ll cover how to build a remote MCP server that reads data from a private data source (a vector store) and makes it available in ChatGPT as a data-only app (formerly called a connector) for chat, deep research, and company knowledge, as well as via API.

Note: For ChatGPT app setup (developer mode, connecting your MCP server, and optional UI), start with the Apps SDK docs: Quickstart, Build your MCP server, Connect from ChatGPT, and Authentication. If you are building a data-only app, you can skip UI resources and just expose tools.

Terminology update: As of December 17, 2025, ChatGPT renamed connectors to apps. Existing functionality remains, but current docs and product UI use “apps”. See the Help Center updates: ChatGPT apps with sync, Company knowledge in ChatGPT, and Admin controls, security, and compliance in apps.
...
To work with ChatGPT deep research and company knowledge (and deep research via API), your MCP server should implement two read-only tools: search and fetch, using the compatibility schema in Company knowledge compatibility.
...
If you are testing this server in ChatGPT as an app, follow Connect from ChatGPT.
...
As someone building a custom remote MCP server, authorization and authentication help you protect your data. We recommend using OAuth and dynamic client registration. For ChatGPT app auth requirements, see Authentication. For protocol details, read the MCP user guide or the authorization specification.
...
Import your remote MCP server in ChatGPT settings.
Create and configure your app in Apps & Connectors using your server URL.
Test your app by running prompts in chat and deep research.
For detailed setup steps, see Connect from ChatGPT.
