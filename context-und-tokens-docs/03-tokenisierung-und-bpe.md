...
## Leitfrage
Wie funktioniert BPE-Tokenisierung, und was bedeutet das für präzise Zählung in NoteDrill?

## Kurzantwort in einfacher Sprache
- BPE ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- TokenizerMismatch muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- Mehrsprachigkeit ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- Codeblöcke sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- JSONLast braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Driftmessung entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Tokenisierung und BPE' und 'BPE' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Tokenizer-Versionen pinnen.
- Mischtext-Testfälle ergänzen.
- Drift je Modell messen.
...
- Das Thema 'Tokenisierung und BPE' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Tokenisierung und BPE' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Tokenisierung und BPE' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
