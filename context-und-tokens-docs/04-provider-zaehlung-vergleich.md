...
## Leitfrage
Wie unterscheiden sich Tokenfelder von OpenAI, Anthropic, Gemini und OpenRouter?

## Kurzantwort in einfacher Sprache
- Feldmapping ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- DeltaTotal muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- CacheBuckets ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- Semantik sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- Normalisierung braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Laufzeitlatenz entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Provider-Zählung im Vergleich' und 'Feldmapping' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Kanonisches Usage-Schema nutzen.
- Delta-vs-Total markieren.
...
- Das Thema 'Provider-Zählung im Vergleich' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Provider-Zählung im Vergleich' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Provider-Zählung im Vergleich' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
