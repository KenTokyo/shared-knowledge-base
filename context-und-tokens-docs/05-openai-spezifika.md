...
## Leitfrage
Welche OpenAI-Mechaniken sind entscheidend für Kontextfenster, Prompt-Caching und Compaction?

## Kurzantwort in einfacher Sprache
- ResponsesAPI ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- PromptCaching muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- CompactFlow ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- ConfigLimits sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- UsageFelder braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Compaction entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'OpenAI-Spezifika' und 'ResponsesAPI' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Compact-Prompt strukturieren.
- Cache-Anteile transparent machen.
- Final-Usage nachziehen.
...
- Das Thema 'OpenAI-Spezifika' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'OpenAI-Spezifika' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'OpenAI-Spezifika' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
