...
## Leitfrage
Wie sollten Claude-Tokenfelder und Context-Window-Regeln in NoteDrill interpretiert werden?

## Kurzantwort in einfacher Sprache
- input_tokens ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- output_tokens muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- cache_read ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- cache_write sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- ContextWindow braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Feldmapping entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Anthropic-Spezifika' und 'input_tokens' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Claude-Felder additiv normalisieren.
- Cache-Auswirkung sichtbar machen.
- Eigene Adaptertests bauen.
...
- Das Thema 'Anthropic-Spezifika' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Anthropic-Spezifika' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Anthropic-Spezifika' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
