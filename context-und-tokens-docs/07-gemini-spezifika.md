...
## Leitfrage
Wie nutzt NoteDrill Gemini-Tokenzählung korrekt, inklusive API-Zählung und Caching?

## Kurzantwort in einfacher Sprache
- countTokens ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- promptTokenCount muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- candidatesTokenCount ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- GeminiCaching sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- ApiKey braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Fallback entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Gemini-Spezifika' und 'countTokens' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- gemini_api priorisieren.
- Exakt-Label bei API-Werten setzen.
- Netzwerk-Fallback sauber zeigen.
...
- Das Thema 'Gemini-Spezifika' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Gemini-Spezifika' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Gemini-Spezifika' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
