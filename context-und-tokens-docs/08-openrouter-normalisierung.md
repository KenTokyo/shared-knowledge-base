...
## Leitfrage
Wie normalisieren wir OpenRouter-Modelle, damit Token- und Kontextwerte konsistent bleiben?

## Kurzantwort in einfacher Sprache
- Modellpräfixe ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- WindowMetadaten muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- FallbackLimits ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- RouterRegeln sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- UnknownRate braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Synchronisation entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'OpenRouter-Normalisierung' und 'Modellpräfixe' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Routenentscheidung protokollieren.
- Unbekannte Fenster klar markieren.
- Metadaten regelmäßig prüfen.
...
- Das Thema 'OpenRouter-Normalisierung' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'OpenRouter-Normalisierung' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'OpenRouter-Normalisierung' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
