...
## Leitfrage
Was ist in NoteDrill heute bereits stabil, und wo liegen die wichtigsten Lücken?

## Kurzantwort in einfacher Sprache
- token-counting-service ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- CTX-Control muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- CondenseMarker ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- Thresholds sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- Routenlogik braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- UIHinweise entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Ist-Stand NoteDrill' und 'token-counting-service' sind direkt mit den bestehenden Codepfaden verknüpft.
...
- Unknown-Handhabung verbessern.
- Schwellen dynamisieren.
...
- Das Thema 'Ist-Stand NoteDrill' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Ist-Stand NoteDrill' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Ist-Stand NoteDrill' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
