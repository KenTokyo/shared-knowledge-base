...
## Leitfrage
Welche UniAI-Muster sind für NoteDrill direkt übertragbar, ohne Architekturbruch?

## Kurzantwort in einfacher Sprache
- context-window ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- limit-management muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- usage-adapter ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- WeakSetGuard sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- continuation braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- run-id entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Ist-Stand UniAI' und 'context-window' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Guard-Muster übernehmen.
- Reset-Pipeline strukturieren.
- Run-ID konsequent verwenden.
...
- Das Thema 'Ist-Stand UniAI' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Ist-Stand UniAI' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Ist-Stand UniAI' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
