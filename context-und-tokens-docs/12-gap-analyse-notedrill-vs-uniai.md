...
## Leitfrage
Welche funktionalen Lücken bestehen zwischen NoteDrill und UniAI beim Kontext-/Limit-Management?

## Kurzantwort in einfacher Sprache
- Zentralisierung ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- GuardReife muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- DriftTracking ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- HandoffSchema sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- ModelOverrides braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Testtiefe entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Gap-Analyse NoteDrill vs UniAI' und 'Zentralisierung' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Gap-Matrix dokumentieren.
- Lücken priorisieren.
- Done-Kriterien messbar machen.
...
- Das Thema 'Gap-Analyse NoteDrill vs UniAI' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Gap-Analyse NoteDrill vs UniAI' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Gap-Analyse NoteDrill vs UniAI' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
