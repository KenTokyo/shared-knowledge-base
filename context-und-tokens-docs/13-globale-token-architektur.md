...
## Leitfrage
Wie bauen wir eine modellübergreifende Token-Architektur, die in Browser, Electron und Mobil stabil läuft?

## Kurzantwort in einfacher Sprache
- KanonSchema ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- AdapterLayer muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- Plattformparität ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- Persistenz sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- ThresholdEngine braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Governance entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Globale Token-Architektur' und 'KanonSchema' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Usage-Vertrag versionieren.
- Plattformgleichheit sichern.
- Feature-Flag-Rollout planen.
...
- Das Thema 'Globale Token-Architektur' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Globale Token-Architektur' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Globale Token-Architektur' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
