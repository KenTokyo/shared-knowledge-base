...
## Leitfrage
Wie müssen Abort-, Stop- und Resume-Pfade aussehen, damit Condense und Sessionwechsel robust bleiben?

## Kurzantwort in einfacher Sprache
- AbortController ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- ReasonCodes muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- Cleanup ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- Checkpoint sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- Parallelität braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Fehlerstrategie entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Abort, Stop, Resume' und 'AbortController' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Reason-Codes vereinheitlichen.
- Cleanup strikt erzwingen.
- Resume nur mit validem Handoff.
...
- Das Thema 'Abort, Stop, Resume' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Abort, Stop, Resume' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Abort, Stop, Resume' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
