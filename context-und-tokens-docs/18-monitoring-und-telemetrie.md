...
## Leitfrage
Welche Messwerte brauchen wir, damit Tokenzählung und Auto-Condense im Betrieb wirklich verlässlich sind?

## Kurzantwort in einfacher Sprache
- UnknownRate ist ein Kernbegriff, weil er die reale Last im Modellfenster beeinflusst.
- DriftMetrik muss sauber getrennt werden, damit Werte zwischen Providern vergleichbar bleiben.
- TriggerErfolg ist hilfreich, darf aber nie still als exakter Wahrheitswert dargestellt werden.
- RunID sollte als Risikoindikator im UI sichtbar sein, sobald das Budget knapp wird.
- Alarmierung braucht eine einheitliche Benennung in Code, Doku und Telemetrie.
- Dashboard entscheidet, ob Auto-Condense stabil auslöst oder zu spät reagiert.
...
- Die Themen 'Monitoring und Telemetrie' und 'UnknownRate' sind direkt mit den bestehenden Codepfaden verknüpft.
- Alle Entscheidungen sollen mit messbaren Abnahmekriterien hinterlegt werden.

## Konkrete Übertragung auf NoteDrill
- Snapshot-Event einführen.
- SLOs definieren.
- Incident-Runbook mit Metriken verknüpfen.
...
- Das Thema 'Monitoring und Telemetrie' profitiert in NoteDrill von derselben Trennung zwischen Datenvertrag und UI.
...
1. Datenvertrag für 'Monitoring und Telemetrie' mit Versionsfeld finalisieren.
...
## Kurzfazit
- Das Themenblatt 'Monitoring und Telemetrie' verbindet Quellenlage, Codebezug und konkrete Umsetzung.
