...
- **Check misst etwas, das der Mutant nicht bewegen kann** — vier Bauarten, gleiche Wurzel: (1) *Eingabe statt Ausgabe* — `stageAhead`/`stageSide` schreibt nur der Klick, marschiert ist `spells.stage`, das der Frame je nach `_hover` neu ableitet; ein Vorher/Nachher auf den Parametern kann nie rot werden. (2) *Vacuous* — `hide()` soll den Pin räumen, aber die Zeilen davor hatten ihn schon gelöst; die Assertion prüft einen Zustand, der vor der geprüften Zeile schon stimmt. (3) *Rückgabewert zweier Zweige* — `cancelPlacement()` gibt aus Pin- und Modus-Zweig `true`; `=== true` überlebt das Abschalten des Pin-Zweigs, weil der Aufruf durchfällt. (4) *Fenster kürzer als die Entprellung* — 8 Frames × 0,016 s = 0,128 s unter `AUTO_CAST_DELAY` 0,16 s, dort sind leading und trailing ununterscheidbar. → Auf den Wert assertieren, den der geprüfte Pfad wirklich schreibt und übergibt; Vorzustand aktiv herstellen statt vorauszusetzen; Rückgabe und Zustand gemeinsam lesen; Zeitfenster mindestens eine Entprellung lang wählen.
  *Falsifier mit 11 Mutationen: 7/11 gefangen, 4 grün geblieben; nach Reparatur nur der Checks 11/11 bei Baseline 188/0 · 2026-08-07*
