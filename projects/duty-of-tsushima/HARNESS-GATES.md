...
- **Ein Gate misst nur, was man ihm zeigt — Gegenstand *und* Bedingung** — zwei Gestalten. **Gegenstand:** „Kamera über Terrain“ prüfte die Spielerkamera, nicht die 14 Benchmarks. **Bedingung:** ein Lesbarkeitsgate maß den Kontrast vom Spawnpunkt (25,4 m) statt vom Ort, an dem der Spieler antwortet (8,1 m) — und hätte damit den Panelentwurf beschuldigt (Schriftgröße, Papierkontrast, Texturauflösung), während der Defekt in der Platzierung lag. Die teure Variante ist nicht der rote Lauf, sondern der grüne: an der falschen Bedingung meldet ein vergrößerter Font grün und deckt die Ursache zu. → Gegenstand in den Zusicherungsnamen („Spielerkamera über Grund“), und **vor** dem Schwellenwert die Frage beantworten, an welcher Bedingung die Zusage fällt; diese Bedingung aus dem Code ableiten (`|Stein − Tafel|`) statt sie ins Gate zu tippen. Entfernungen, an denen die Zusage *nicht* fällt, bleiben messbar und werden ausdrücklich kein Tor.
