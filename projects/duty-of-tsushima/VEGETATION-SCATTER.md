...
- **Ein Löser, der eine Kamera gegen das Gelände freistellt, kennt den Bewuchs nicht — und eine Kugel um die Baummitte legt dessen Masse auf die falsche Höhe** — `cameras.js` garantiert seit dem Tag, an dem sieben von vierzehn Kameraaugen unter der Erde lagen, Luft unter jedem Auge; gegen das Höhenfeld. Nachdem eine Phase aus vier flachen Ikosaedern echte Kronen gemacht hatte, standen zwei von fünfzehn Benchmarkkameras im Laub (91,5 % und 76,9 % des Bildes) — derselbe Fehler eine Ebene höher, mit demselben Symptom: eine Kamera im Bewuchs fällt nicht aus, sie liefert ein plausibles Bild, und jede Farbzahl darauf beschreibt ein Blatt. Der naheliegende zweite Löseschritt modellierte jede Pflanze als **Kugel um ihre Mitte, gerechnet vom Fußpunkt** — und meldete beide Kameras gelöst, während das unabhängige Gate eine davon mit 69,4 % las. Eine Kiefer ist ein Stamm mit Last obendrauf: die Kugel endete 1,5 m unter dem Wipfel, und die betroffene Kamera schaut von oben **auf Wipfel** in 1,6 bis 5,1 m. → Kronenmasse als **Band** führen (Kronenfuß bis Oberkante) und den Kronenfuß aus der gezeichneten Geometrie messen — die niedrigste Höhe, auf der die Form einen Anteil ihrer vollen Ausladung erreicht, dieselbe Größe, die das Wind-Attribut ohnehin schreibt. Getestet wird dagegen ein senkrechter Zylinder mit **beiden** Schnittpunkten: wer über einer Krone steht und nach unten sieht, trifft nur den fernen. Der Stamm darunter bleibt Luft, und Luft ist, was die Kamera dort auch sieht.
  *`plateau-hero` 91,5 → 9,7 %, `boss-8` 76,9 → 5,9 % Kronenanteil unter 8 m; 400 von 400 Treffern lagen im obersten Fünftel der Pflanze, also nicht am Stamm · `src/world/foliage.js:crownBase`, `src/world/cameras.js:canopyShare` · `89bee7b` · 2026-08-04*

- **Eine Kamera aus dem Kronendach zu heben macht das Bild erst schlechter** — der Sweep sieht aus wie ein Fehler und ist die Physik der Sache: 91,5 % Kronenanteil ungehoben, **97,9 % einen Meter höher**, 96,7 bei +2, dann 83,9 · 56,1 · 29,7 · 9,7 (+6) · 1,7 · 0,2. Unter dem Dach sieht ein Auge zwischen Stämmen hindurch; auf Kronenhöhe sieht es die Unterseite des Daches, und erst darüber wird es frei. → Einen solchen Deckel in kleinen Schritten **abgehen** und den ersten Durchbruch nehmen. Bisektion und jeder Abbruch auf „wird besser" halten am Maximum an, nicht an der Lösung, und ein Kommentar, der Monotonie behauptet, weil die erste Messreihe monoton aussah, macht daraus eine Zusage. Die erste Fassung hier tat genau das — die Monotonie stammte aus dem falschen Kugelmodell.
  *`boss-8` dieselbe Form: 76,9 → 92,0 → 92,2 → 85,2 → 43,4 → 14,1 → 1,4 → 0,0 % über +0…+7 m · `src/world/cameras.js:clearCanopy` · `89bee7b` · 2026-08-04*

- **Ein Schnittkarten-Sprite, dessen Bilddeckung unter eine Handvoll Pixel fällt, wird zum gefüllten hellen Rechteck — und der Spieler meldet es als Rendering-Fehler** — vier Playtests in Folge stand „komische Rendering-Fehler, helle Rechtecke in der Luft" im Protokoll, und dreimal wurde in der Renderschicht gesucht (Framebuffer-Rückkopplung, Post-Kette, Instanzmatrizen). Es war das fallende Laub. Der Beweis kam nicht aus dem Code, sondern aus dem Bildschirmfoto des Spielers: isolierte helle Cluster gegen ihre Umgebung suchen, clustern, RGB mitteln — 200,194,198 ist exakt `uPetal` (1.0, 0.80, 0.84) nach Beleuchtung, 216,141,98 exakt `uMaple` (0.86, 0.26, 0.10). Keine andere Quelle im Baum trägt diese zwei Farben. Der Mechanismus ist die Alphaschwelle: `if (t.a < 0.42) discard` schneidet bei Mipstufe 0 eine Blattsilhouette und bei fünf Bildpunkten nichts mehr, weil die Randtexel über die Mipkette zur Mitte hin weggemittelt sind. Übrig bleibt die volle Karte in voller Helligkeit, weil die Fernblende erst weiter draußen einsetzt. → **Jede** alphageschnittene Karte braucht eine Blende auf ihre Bilddeckung, nicht nur auf ihre Entfernung: `px = size / dist * projectionMatrix[1][1] * viewportHeight * 0.5`, darunter ausblenden. Und: bevor irgendwer den Renderer verdächtigt, das gemeldete Bild pixelweise vermessen — die Farbe eines Artefakts ist eine Signatur, und Uniformen sind durchsuchbar.
  *Blattkarte 1,35 × `aParam.y` = 7,4 bis 15,5 cm, auf 22 m mit 1/tan(27,5°) = 1,921 also 5 bis 6 px — genau die gemessenen 6x5-Cluster. Blende 2,5 → 6,0 px, `LEAF_RANGE` 44 → 28 m, `COUNTS.leaves` 3150 → 1900 gegen die um (44/28)² gestiegene Nahdichte · `src/render/Life.js` · 2026-08-07*

- **Ein `fade`, das nur in den Frühausstieg des Vertexshaders geht, blendet nichts aus — es schaltet** — dieselbe Datei rechnete drei Ausblendungen (Lebensphase, Ferne, Nähe) zu einem Faktor zusammen und benutzte ihn ausschließlich für `if (fade <= 0.02) return;`. Bei einer **opaken** Schnittkarte gibt es keinen Deckkraftkanal, in dem der Wert landen könnte, also ist jede dieser drei Rampen in Wahrheit eine harte Kante bei 0,02 gewesen — auch die, die der Dateikopf ausdrücklich als „blendet aus, bevor es das Gras erreicht" beschreibt. Ein Kommentar hat vier Monate lang eine Wirkung behauptet, die der Code nie hatte. → Bei opaken Karten wandert die Blende in die **Alphaschwelle**: `mix(1.02, 0.42, vFade)`. Bei vollem `fade` steht wieder exakt die alte Schwelle, sinkend frisst sie die mip-gemittelten Randtexel und die Karte zerfällt von außen. Nah wirkt das fast nicht, weil Mipstufe 0 nahezu binäres Alpha hat — die Verteilung, die man ohnehin will.
  *Fiel erst auf, als die Bilddeckungs-Blende oben eingebaut wurde und messbar nichts tat · `src/render/Life.js:LEAF_FRAG` · 2026-08-07*
