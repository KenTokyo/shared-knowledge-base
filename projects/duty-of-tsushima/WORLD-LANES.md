...
- **Ein Anziehungsterm findet keine Route, er bevorzugt nur eine** — ein autorisierter Weg im Splat-Kanal mit Bonus im Kostenterm wird trotzdem verlassen, sobald ein zweiter Term teurer wird als der Bonus: Abweichung von der Sehne kostet linear in der Lauflänge, der Bonus ist konstant je Schritt. Ab dem Meter, wo beide sich kreuzen, ist die gebaute Straße rechnerisch unerreichbar — mit jeder Bonus-Höhe, die den Rest des Feldes nicht kaputtmacht. → Gebaute Wege **ablaufen** statt nachlösen, mit denselben harten Grenzen; gelösten und abgelaufenen Kandidaten gegeneinander antreten lassen und die weitere Strecke nehmen, dann braucht keiner Vorrang nach Herkunft.
  *`ROAD_MAIN` bricht auf 137° auf, Lane peilt 180°: nach 20 m kostet `drift` 0,22 × 14,7 = 3,2 gegen `path`-Bonus 2,2. Lane `road` 26,0 → 91,9 m, 190 statt 27 Wirbelsäulenproben · `src/world/kazelanes.js:walkRoad` · `3a19be1` · 2026-08-06*

- **Die Anfangspeilung eines Weges sagt nicht, wohin er führt** — ein Abstieg aus einem Kessel ist eine Serpentine, und eine Serpentine bricht nicht dorthin auf, wo sie ankommt. Wer einen autorisierten Weg über seine ersten Meter einer Richtung zuordnet, ordnet ihn dem falschen Verbraucher zu; wer über das erste Segment misst, misst zusätzlich die Unterteilung des Splines statt des Geländes. → Die **Sehne bis zur eigenen Reichweite des Verbrauchers** lesen, nicht den Aufbruch und keine feste Leselänge. Zusätzlich paarweise exklusiv vergeben, sonst nehmen zwei Verbraucher denselben Weg und sind danach derselbe Weg.
  *Dieselbe `ROAD_MAIN`: Sehnenpeilung 136,8° über 24 m, 142,9° über 40 m, 215,7° über 90 m. Über 24 m gelesen fiel die einzige gebaute Straße an `shrine` (Lücke 1,8° gegen 43,2°), über die Reichweite an `road` (35,7° gegen 41,8° von `cascade`) · `kazelanes.js:chordBearingAt` · `3a19be1` · 2026-08-06*

- **Eine Strafe im Kostenterm kauft man mit einem anderen Term frei** — der fremde Vertrag `nav.SLOPE_LIMIT` 0,62 stand im Lane-Löser nur als Gewicht, also nahm der Marsch eine Wand, wenn sie sonst billig rechnete. Kein Gate fiel darauf: das Begehbarkeits-Gate misst die Stufe über ein 2-m-Fenster, `nav.walkable()` den örtlichen Hang — zwei Größen, und die grüne beantwortet die andere Frage nicht. → Grenzen fremder Subsysteme als **harte Ablehnung** führen, nicht als Gewicht, und entlang des Schrittes lesen statt am Landepunkt; die Kante liegt in der Mitte.
  *`cascade`: 19 % der Wirbelsäule über 0,62, **5,3 m am Stück**, Spitze 0,78 — bei grünem Gate. Danach 0,0 m und Spitze 0,57, Preis 42,7 → 34,4 m Länge · `kazelanes.js:worstSlope` · `3a19be1` · 2026-08-06*
