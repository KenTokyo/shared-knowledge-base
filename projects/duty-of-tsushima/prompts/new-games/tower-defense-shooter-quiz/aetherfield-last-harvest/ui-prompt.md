# Aetherfield: Last Harvest — UI Build Prompt
...
```text
CREATE AETHERFIELD: LAST HARVEST — COMPLETE UI AND PLAYER FLOW

MISSION

Build the complete shippable interface for an original elevated-camera 3D farm-defense shooter quiz.
The interface must feel warm, premium, tactile, modern, and practical.
...
Target the warmth and material storytelling of a handcrafted fantasy field journal.
...
- The camera is elevated three-quarter 3D, never a static board view.
...
- Name: `AETHERFIELD: LAST HARVEST`.
- Base surface: charcoal ink with warm parchment translucency.
- Structural rails: worn brass with subtle edge variation.
- Neutral selection: indigo.
- Valid, charged, correct, and friendly: restrained aether cyan.
- Invalid, wrong, core-critical, and hostile: vermilion.
- Gold resource: warm amber, never neon yellow.
- Insight resource: pale cyan-white with a small faceted seed symbol.
- Core Seeds: coral-gold seed facets.
- Data typography: condensed humanist sans with open counters.
- Title and quiz question may use one restrained readable serif.
- Never place textured parchment directly behind small text without an opaque backing.
- Never use glow as the only boundary between UI and world.
- Never use cyan as a decorative wash.
...
- Preserve the central 55% of the image for aim, enemies, lanes, towers, and quiz pylons.
...
- Show the title over a live or representative Amberstep Farm view.
...
- Selection state uses brass frame plus restrained cyan mechanism light.
...
- Ranger and Technician use the same geometry but distinct class material treatment.
...
- Selected slot uses amber/brass.
- Valid ghost uses cyan edge plus physical translucent material.
...
- Correct state flashes cyan/white, grants Insight, and opens a three-card buff choice.
...
- Selection is a soft brass tick.
