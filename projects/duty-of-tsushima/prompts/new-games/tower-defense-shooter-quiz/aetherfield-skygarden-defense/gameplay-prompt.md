# Aetherfield: Skygarden Defense — Gameplay Build Prompt
...
```text
CREATE AETHERFIELD: SKYGARDEN DEFENSE — COMPLETE GAMEPLAY AND WORLD

MISSION

Build a complete original browser action tower-defense shooter quiz across a large bright voxel fantasy farm.
...
World composition should carry bird's-eye tactical clarity, broad farm acreage, perspective depth, and graphic shadows.
Tower-defense readability should expose lane, range, target, contact, and consequence at a glance.
Characters must read as chunky voxel builds at gameplay distance while using real articulated full-body motion.
...
- A large working farm owns at least 65% of the ordinary gameplay view before defenses.
- The pale stone Quiz Core is the central protected landmark.
- One open gate-to-Core route is immediately legible.
- The selected hero is a tiny but bold voxel participant with cuboid head, hands, feet, equipment, and readable motion.
...
- One complete initial map: `SKYGARDEN EXPANSE`.
...
WORLD — SKYGARDEN EXPANSE

- Shape the map as four large readable farm plots around a central wind-fed archive terrace.
...
- Create one broad traceable S-curve lane through crop bands, orchard rows, irrigation, and perimeter sheds.
- Keep the minimum navigation corridor at two metres.
- Use low fence breaks and gates for player traversal.
- Use broad terraces and drainage bands for depth without breaking the whole-farm overview.
- Include one windmill, two perimeter barns, one orchard belt, one irrigation wheel, and one field workshop as landmarks.
- Keep skyline calm: rolling hills, distant grain, sparse trees, no giant fantasy city, palace, or ornamental plaza.
...
- Timber uses broad grain bands, chunky joinery, simple wear, and clear soil contact.
- Pale stone uses large block courses, softened chips, and stable clean midtones.
- Brass uses broad dull surfaces and restrained oxidation; it is never mirror gold or covered in tiny fittings.
- Blue ceramic uses thick simple forms, large glaze regions, and a few readable fractures.
- Crops use bold species bands, chunky clusters, stepped height groups, and layered wind.
- Cloth uses broad shapes and restrained movement without fine weave noise.
- Water uses simple sky color, ripples, and depth bands without mirror reflections.
- Main light is warm sun.
- Sky and shadows carry cool fill.
- Mint aether remains local to active mechanisms.
...
- Avoid uniform ambient light, crushed shadows, excessive bloom, milky fog, photoreal microtexture, and wet cinematic surfaces.

SKYGARDEN STYLE GUARDRAILS

- Ordinary gameplay camera pitches 60–65 degrees downward from horizontal.
- Farm ground, crops, irrigation, routes, fences, and open build space cover at least 65% of the view.
- Hero ordinary frame height stays 4–6%; bosses stay below 16% unless a short introduction is active.
- Character heads, torsos, pelvises, hands, feet, weapons, packs, and armour use clearly stepped voxel volumes.
- Structures use a few broad primary masses, visible mechanics, simple silhouettes, and limited surface noise.
- Keep trees and buildings near the perimeter so they never turn the overview into a close third-person corridor.
- Preserve warm fantasy craft, but reject realistic humans, tiny greebles, ornamental temple forms, and decorative city density.

CAMERA

- High bird's-eye perspective camera.
- Pitch range target 60–65 degrees downward from horizontal.
- Maintain perspective; do not use orthographic projection.
- Hero ordinary frame height target 4–6%.
- Follow position smoothly with a broad dead zone that protects the whole-farm overview.
- Aim cursor shifts framing slightly toward target direction.
- Zoom supports tactical near/far bounds but never reaches a shoulder-level chase view or reveals outside world gaps.
...
- Use a compact articulated voxel rig with obvious cuboid volumes, not a realistic miniature or one solid block.
- Include root, pelvis, chest, cuboid head, shoulders, elbows, wrists, hips, knees, ankles, broad block hands, and broad block feet.
...
- Silhouette: cuboid straw helmet, square indigo field jacket, stepped pale armour, block hands, broad boots, brass tool pack, chunky firearm.
...
- Silhouette: square ceramic gauntlets, blocky lavender short coat, broad boots, sky-aether canisters, tiny voxel survey drone.
...
- Build Skygarden terrain, landmarks, Core, gates, and routes.
...
- Skygarden Expanse is a coherent working farm and tactical battlefield.
