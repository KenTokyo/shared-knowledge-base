# Cloud Flakes Style — reaktives Heightfield und Clipmap
...
**Map-Fokus:** Gebundenes Deformationsfeld, kontinuierlicher Terrain-LOD und warm-kühle Reliefbeleuchtung.
**Sichtprüfung:** Keine automatische Browser-/Screenshot-/Gameplayprüfung ohne aktuellen ausdrücklichen Userauftrag.

```text
CREATE SKYGLASS & VERDANT ENDLESS VOXEL SLASHER — CLOUD FLAKES STYLE REACTIVE TERRAIN
...
CLOUD-FLAKES-STYLE-MAPARCHITEKTUR

1. Übernimm stackneutrale Prinzipien aus `D:/CODING/React Projects/github-repos-examples/Claude-Flakes`; kopiere keine Babylon.js-/WGSL-API, Shaderdatei, Zahlentabelle oder fertige Spielwelt.
2. Referenzbar ist eine terrain-dominante Welt, in der Makroform, feine Oberfläche, Licht, Bewegung und Skillrückstände als geschlossenes System reagieren.
3. Baue pro Welt ein gebackenes Makro-Heightfield als Bodenwahrheit:
   - breite authored Landformen und Kampfflächen,
   - gerichtete mittlere Oberflächenformen statt isotropem Noise,
   - Material-/Steigungs-/Expositionshilfsfelder aus finalem Bake,
   - CPU-Spiegel oder gleichwertige Abfragequelle für exaktes Grounding.
4. Skyglass und Verdant behalten Komposition, Architektur und Materialidentität; Cloud Flakes Style ist Map-Technik und Oberflächenwirkung, kein Zwang zu Schneebiom oder Babylon-Look.
5. Skyglass formt Plateau, Plaza, Kanäle, Palastpad, Aquäduktfundamente und Canyonrand im Heightfield; Architektur sitzt konstruktiv darauf.
6. Verdant formt Kessel, Lichtung, Wege, Hallenpad, Wurzelhügel, Terrassen und Randanstieg im Heightfield; Titanbäume bleiben authored Landmarken.
7. Nutze für kontinuierlichen Boden ein statisches Nested-Ring- oder gleichwertiges Clipmap-Mesh nur, wenn Zielkamera damit wirklich unterschiedliche Auflösung braucht.
8. Clipmap-Vertices tragen kompakte Gitter-/Leveldaten; Vertexpfad setzt Weltposition, Raster-Snapping, Morphing, Makrohöhe und Reaktion ohne CPU-Neubau pro Frame.
9. Überlappungs-/Morphband verhindert Risse zwischen Auflösungsringen; Reaktionsform wird nach aktueller Gitterweite bandbegrenzt, statt in wandernde Dreiecksgrate zu zerfallen.
10. Zentriere Detail- und Reaktionsautorität am Spieler/Playfieldfokus, nicht blind an Orbitkamera; reine Kameradrehung darf bestehende Narbe weder bewegen noch umformen.
11. Feine Oberflächenstruktur lebt frequenzgerecht:
   - Makroform im Bake,
   - mittlere gerichtete Reliefschicht nur bei ausreichender Vertexdichte,
   - Mikrodetail und Materialnormalen im Fragmentpfad,
   - pixel-/footprintbasierter Filter gegen Flimmern und Moiré.
12. Baue ein begrenztes, texelgesnapptes `SurfaceReactionField` als Ping-Pong- oder gleichwertiges Weltzustandsfenster um Spieler/Arena.
13. Feldkanäle sind materialsemantisch, nicht schneespezifisch:
   - Nettovertiefung,
   - verdrängte Masse/hochgedrückter Rand,
   - Verdichtung oder Bruchgrad,
   - signierter Oberflächenzustand wie feucht/vereist gegenüber verbrannt/freigelegt.
14. Ein fester Brush-Stagingpuffer sammelt alle Kontakte eines Frames; harte Kapazität, billiger Außenfenster-Reject und Überlastpriorität verhindern Wachstum.
15. Ein Simulationsschritt übernimmt Scroll/Recycle, langsame materialabhängige Erholung, gerichtete Verwitterung, neue Brushes und sichere Clamps; keine separate Objektliste pro Spur.
16. Weltadressierung bleibt toroidal oder gleichwertig recyclebar; neu sichtbare Zellen werden sicher geleert und Feldränder vor Naht weich ausgeblendet.
17. Materialprofile reagieren verschieden:
   - Skyglass-Stein: flache Vertiefung, scharfer Bruch, Splitterrand, Staub/Hitzezustand, sehr langsame Heilung,
   - Verdant-Boden/Moos: tiefere Verdichtung, weicher Erdwall, aufgerissenes Grün, Feuchte, schnellere natürliche Erholung,
   - Holz/Wurzel: oberflächliche Faserkerbe ohne Terrainloch,
   - Wasser: Wake/Schaum/Trübung statt persistenter Geometrienarbe.
18. Beauty-, Depth-/Prepass- und alle Schattenvertexpfade verwenden dieselbe Clipmap-Platzierung und denselben gefilterten Feldoffset; Narben schreiben echte Tiefe und werfen passende Schatten.
19. Fragmentmaterial liest Feldgradient, Randmasse und Zustand für Normalen, Albedo, Rauheit und gebrochene/verdichtete Oberfläche; Materialfarbe allein darf keine tiefe Furche vortäuschen.
20. CPU-Gameplayhöhe bleibt aus Baseline-Bake stabil, solange Rückstände kosmetisch sind; VFX und Partikel dürfen sichtbaren Feldoffset zusätzlich abfragen, ohne Navigation umzuschreiben.
21. Reset speist beide Ping-Pong-Seiten aus sicherem Nullzustand; Weltenwechsel, Neustart und deterministische Reviewzustände erben keine alten Krater.
22. Auflösung und Abdeckung aus maximal 600 Einheiten, Kameradistanz, kleinster sichtbarer Narbe, VRAM und Zielhardware ableiten; keine 4.096²-/2.048²-Referenzwerte blind übernehmen.
23. Feldupdate allokiert nichts pro Frame; Brushdaten, Targets, Scratchwerte und Skilllichter sind vorab begrenzt und aufgewärmt.
24. Eine zentrale Environment-SSoT liefert Sonnenrichtung, direkte Radiance, Himmels-/Umgebungslicht, Wind, Nebel, Schatten und Grade an jeden Materialconsumer.
25. Lichtwirkung orientiert sich an Cloud Flakes:
   - niedrige warme Streifsonne zeichnet Relief und lange Schatten,
   - kühler Himmel trennt Schatten vom warmen Direktlicht,
   - Wind-/Reliefrichtung bleibt quer genug zur Sonne, damit Mikrokanten nicht flach beleuchtet werden,
   - Luftperspektive gibt Feldmaßstab, ohne Nahkampf milchig zu machen.
26. Direkte Sonne bewegt sich kontinuierlich; teure Himmel-/IBL-Bakes dürfen kontrolliert und schwellenbasiert folgen, ohne Schatten oder Speculars sichtbar springen zu lassen.
27. Kaskadierte oder passende gerichtete Schatten werden an echte Kamerahülle und gemessene Höhenextents angepasst sowie im Welttexelraster stabilisiert.
28. Gepoolte Skilllichter beleuchten deformierte Mapmaterialien im selben HDR-Maßstab; feste Slots verhindern Shadervarianten beim ersten Cast.
29. Belichtung und Tone Mapping bewahren helle Skyglass-Steine, Verdant-Lichtkegel und Skillkerne gleichzeitig; Bloom verstärkt HDR-Kerne, ersetzt keine Relief- oder Materiallesbarkeit.
30. Visuelles Ziel: weite zusammenhängende Geländeoberfläche mit gerichteter Mikrostruktur, warm-kühler Lichttiefe und einer Arena, die nach einer Welle sichtbar benutzt aussieht.
...
3. Makro-Heightfield, kontinuierlichen Boden und SurfaceReactionField bauen.
4. Skyglass als vollständigen reaktiven Terrain-Vertical-Slice bauen.
...
8. Verdant als zweites Materialprofil derselben reaktiven Terrainengine bauen.
...
- beide Karten nutzen dieselbe Cloud-Flakes-artige Heightfield-/Clipmap-/ReactionField-Grundlage mit eigenen Materialprofilen.
