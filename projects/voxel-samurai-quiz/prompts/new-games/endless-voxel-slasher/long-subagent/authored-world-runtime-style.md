# Authored World Runtime — Subagent Orchestration
...
- Execution profile `orchestrated`: one integration owner keeps world spec, bake dependencies, runtime, Environment, camera, final image, coupled architecture, and shared state. Sub-agents own disjoint paths only: world specs/bakes, world rendering/environment/camera, characters/combat/VFX, gameplay/UI/audio, or performance/accessibility. No two agents edit the same runtime owner, shared field, task section, or integration seam. Each sub-agent returns changed files, evidence, risks, and unresolved gaps; only the integration owner merges decisions and performs the complete regression review.
