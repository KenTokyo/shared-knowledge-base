...
*   **Rule 5.25 (End-to-End Field Wiring Checklist):** 🚨 **DATA-INTEGRITY RULE** – Wenn neue Domain-Felder oder Tabellen eingeführt werden (z.B. `dailyEntries.stressLevel`, `customFieldValues`), MÜSSEN sie in einem Schritt durch alle Schichten verdrahtet werden: (1) DB-Schema + Constraints, (2) alle Schreib-Actions (`createEntry`, `updateEntry`, etc.), (3) alle Lese-Finder/Loader, die diese Daten in die UI bringen, (4) State-Manager Hooks/Contexts (z.B. `usePermanentEntryState`), (5) Edit- und Read-Only-Components. Vor Abschluss eines Features IMMER einen kurzen End-to-End-Check für jedes neue Feld durchführen, um „im Edit-Mode sichtbar, im Read-Only-Modus verschwunden“-Bugs zu verhindern.

*   **Rule 5.26 (Overview Parity For Key Metrics):** 🚨 **CRITICAL DATA VISIBILITY** – Wenn eine Domäne neue oder geänderte Kernmetriken speichert (z.B. Übungsgewicht, Kalorien-Summen), MUSS jede Übersicht/Compact-Card dieselben Kernwerte wie der Detail-View anzeigen. Nach Feldänderungen: Paritäts-Check durchführen (DB → Service-Mapping → State-Manager → Summary-Komponenten) und sicherstellen, dass Einheiten/Formatierung erhalten bleiben, damit Werte nicht stillschweigend in kompakten Ansichten verschwinden.
