...
- **Pixelmaße über verschiedene Auflösungen verglichen** — Native Pixel wirken ähnlich, relative Bildgröße stimmt nicht. → Längen, Flächen und Crops in Frameanteilen normieren; FOV/Aspect und Zielauflösung mitführen.
...
- **Messfenster trägt falsches Subjekt** — Cropname stimmt; Pixel gehören Blüte, Sonne, Figur oder Kontrolle. → Crop einmal im echten Frame verorten; Subjektanteil, Population und Kontrollcrop ausgeben, bevor eine Spalte steuern darf.
...
- **A/B liegt unter eigenem Rauschboden** — Wiederholungen bestätigen nur gleiche Sitzung oder Vorgeschichte. → Zuerst unverändertes A/A; A/B unmittelbar paaren und in Gegenreihenfolge messen; Look, Seed, Warm-up, Fremdlast und Vorgeschichte gleich halten.
...
- **Messfenster endet an A/B-verschobenem Ereignis** — Beide Zahlen stimmen, decken aber verschiedene Arbeit ab. → Ganze Phase oder feste Besitzgrenzen messen; Marken beidseitig setzen, Bestand und Delta getrennt führen.
...
- **Durchschnitt beantwortet Verteilungsfrage** — Peak, Fast-Schwarz, Stutter oder lokaler Defekt verschwindet im Mittel. → Histogramm/Quantile und Population ausgeben; Framekosten mindestens p50/p95/p99/Worst, Bildkanäle mit Unter-/Überlaufanteil.
...
- **Instrument misst Nachbarfrage** — FPS im Warm-up gilt als GPU-Kosten; Rendererzähler enthalten Vorgängerframes; Offline-Modell umgeht Shipping. → Fehlergate, Bildrate, GPU/CPU-Kosten und Produktintegration getrennt messen; jedes Instrument mit Null- und Positivkontrolle vergiften.
...
- **Schranke über leerer Population grün** — `every`, `===0` oder „keine Naht“ wird besser, wenn Subjekt fehlt. → Population/Nenner an dasselbe Verdict binden; Kamera oder Quelle absichtlich entfernen und Rot verlangen.
