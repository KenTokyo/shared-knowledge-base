...
Run vinext check to scan for compatibility issues
Install vite, @vitejs/plugin-react, and App Router-only deps (@vitejs/plugin-rsc, react-server-dom-webpack) as devDependencies
Rename CJS config files (e.g. postcss.config.js -> .cjs) to avoid ESM conflicts
Add "type": "module" to package.json
Add dev:vinext and build:vinext scripts to package.json
...
CLOUDFLARE_API_TOKEN env var (CI / non-interactive) — create a token at dash.cloudflare.com/profile/api-tokens using the Edit Cloudflare Workers template. That template grants all the permissions vinext deploy needs.
Account ID:
...
Benchmarks Dashboard	Build performance tracking over time (D1-backed)	benchmarks.vinext.workers.dev
...
Existing process.env values (shell/CI)
.env.<mode>.local
...
Why the bundle size difference?
